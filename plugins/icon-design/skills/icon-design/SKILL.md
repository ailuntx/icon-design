...
For a new or replacement app icon, treat concept choice as a user decision unless the user explicitly delegates it or has already selected a direction. After generating candidates, show one labeled comparison at 32px and 64px, state a recommendation with its weakness, and ask the user to choose or request changes via `request_user_input_async` when available. Wait for that answer before refining a candidate, creating the final Composer document, replacing App assets or rebuilding the App. Authorization to use a paid image provider chooses a production method; it does not select the visual direction.
...
Start API concept exploration at low quality and 1024 square when supported. Save usage cost returned by the provider. Fewer generations and lower generation quality can reduce cost; resizing or compressing an already-generated image does not refund generation cost. A failed or timed-out request may still have incurred a charge; do not automatically retry. Before trying image edits or reference uploads, verify that the selected provider accepts the CLI's request format; a minor color adjustment does not justify an unverified paid edit call. Increase quality for the selected direction when needed, within the user's cost authorization.
...
Import aligned SVG or transparent PNG layers into the real Icon Composer app when available, after the direction is chosen. A background plus one raster foreground is a valid minimal document, but its internal folds are not independently editable layers. For fuller Liquid Glass control, reconstruct or regenerate flat, opaque, separately editable shapes; do not apply baked glass effects twice. Validate native opening, layer presence, appearances and a flattened export. For repeated previews of the same validated document, use the installed `ictool` CLI where it preserves the intended export; reserve UI work for creating/editing the document and checking the final native result. If Composer cannot be run, deliver importable layers and instructions and clearly state that native validation is pending. Do not fabricate an undocumented `.icon` JSON schema.
...
Before replacing an App icon, inspect both build asset references and launch-time icon overrides (for example `NSApp.applicationIconImage`). After rebuilding, verify the running App path and its actual Dock icon before reporting success; a correct bundled ICNS or successful launch alone does not prove the visible icon changed.
